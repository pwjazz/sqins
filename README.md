...
sqins maps fields to database columns on a one-to-one basis, converting between the Scala type and the appropriate database type.

Fields are mapped to and from the database using TypeMapping objects.  sqins provides TypeMappings for the common basic types, and you can easily add your own.
...
sqins uses `TypeMapping` implicitly.  All constructors and methods that require a `TypeMapping` (for example `Column`) accept implicit TypeMappings.  By importing `org.sqins.Implicits._` you import all of the built-in implicit `TypeMapping`s, so you usually don't need to reference them explicitly.
...
Since the usual style is to use `TypeMapping` implicitly, you should define your own `TypeMapping`s as implicit as well.

A `TypeMapping` for a regular type extends the `TypeMapping[type]` trait and implements the `_get` and `_set` methods.
